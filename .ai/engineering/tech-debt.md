...
| D-016 | `app/globals.css` (brand color tokens) | Verified against Avanse's own brand guidelines | Colors were sourced from Brandfetch, a third-party brand-asset aggregator, since no official Avanse brand guide was available in this environment | Low — cosmetic only | If the demo goes in front of anyone who'd notice an off-brand teal | Get the real guidelines and swap 3 hex values in `globals.css` | open |
| D-015 | `lib/segmentation.ts` | Cohort/persona segmentation is a fixed rule-based taxonomy, not a trained or marketing-validated segmentation model | Built directly from existing profiling fields per user request; no real campaign performance data exists yet to validate that these particular persona boundaries predict conversion | Low — purely additive/display, doesn't gate any workflow | If Marketing wants personas tied to actual campaign response data | 2-3 days once real conversion data exists to inform the taxonomy | open |
| D-014 | `lib/conversation/sessions.ts`, `lib/propensity.ts`, `lib/conversation/orchestrator.ts` | Sentiment classification and session-boundary detection (ADR-007) | Sentiment comes from the same Gemini call that drafts the reply (asked to self-report, not a dedicated classifier); a session boundary is purely a time gap, not an explicit "goodbye"/topic-change signal. In testing, Gemini frequently omitted `sentiment`/`sessionNote` from `submit_reply` despite both being listed as required — added a keyword-based fallback (`classifySentimentFallback`) so the fields are never silently empty, but the AI-classified value should be treated as best-effort, not guaranteed | Medium — self-reported sentiment from a reply-drafting model is weaker evidence than a dedicated classification pass, the fallback is cruder still, and a fixed time gap will occasionally split or merge sessions oddly | Lead-temperature accuracy and the Leads CSV Sales relies on | If Sales reports the Hot/Warm/Cold labels don't match reality | 2-3 days for a dedicated sentiment pass; session-boundary heuristics need real usage data to tune | open |
